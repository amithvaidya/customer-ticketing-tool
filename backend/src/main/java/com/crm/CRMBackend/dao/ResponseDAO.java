...
public class ResponseDAO {
	...
	public int updateResponse(Response response){
		...
		Response tempResp = jdbcTemplate.queryForObject("select * from responses where id=?", Response.class);
			...
			if(response.getTicketId()!=null) tempResp.setTicketId(response.getTicketId());
			if(response.getAgentId() != null) tempResp.setAgentId(response.getAgentId());
			if(response.getCustomerId()!=null) tempResp.setCustomerId(response.getCustomerId());
			if(response.getMessage()!=null) tempResp.setMessage(response.getMessage());
			...
			if(response.getResponseByAgent()!=null) tempResp.setResponseByAgent(tempResp.getResponseByAgent());
		
		return jdbcTemplate.update(
			"update response set ticket_id=? agent_id=? customer_id=? message=? created_timestamp=? ticket_status=? response_by_agent=? where id=?",
